		...
		SimpleNotification.info(
			{
				...
				image: mmdImage,
				text:
					"It looks like it is your first time using MyMangaDex, for any help go on {{github|https://github.com/Glagan/MyMangaDex}}.\nDon't forget to look at the **Options** !",
				buttons: {
					value: 'Open Options',
					type: 'message',
					onClick: (n) => {
				...
				SimpleNotification.info(
					{
						...
						image: mmdImage,
						text:
							"You can now **Confirm Updates** that would be rejected (lower chapter) and update a title only if it's in your **MangaDex** list.\nYou can enable these options... in the **options**.\nIf you have a problem, don't forget: **Github**, **Reddit** or **Discord**.",
						buttons: {
							value: 'Open Options',
							type: 'message',
							onClick: (n) => {
				...
				SimpleNotification.info(
					{
						...
						image: mmdImage,
						text:
							'**MyMangaDex** online save has been updated and now save your **options** and your **history** (if enabled).\nIf you have **any** problem, same as the previous notification.',
						buttons: {
							value: 'Open Options',
							type: 'message',
							onClick: (n) => {
				...
				SimpleNotification.info(
					{
						...
						image: mmdImage,
						text:
							'Version **2.3** come with more options, you can now display the full size cover in the thumbnails, hide higher chapters and highlight the next chapter.\nSee you in the options !',
						buttons: {
							value: 'Open Options',
							type: 'message',
							onClick: (n) => {
...
async function updateLocalStorage(manga, options) {
	...
	if (
		...
		manga.currentChapter.chapter > manga.lastMangaDexChapter
	) {
...
function processMyAnimeListResponse(manga, text) {
	...
	// Start date
	manga.start_date = {};
	manga.start_date.month =
		parseInt(/add_manga_start_date_month.+\s.+value="(\d+)?"\s*selected="selected"/.exec(text)[1]) || '';
	manga.start_date.day =
		parseInt(/add_manga_start_date_day.+\s.+value="(\d+)?"\s*selected="selected"/.exec(text)[1]) || '';
	manga.start_date.year =
		parseInt(/add_manga_start_date_year.+\s.+value="(\d+)?"\s*selected="selected"/.exec(text)[1]) || '';
	// Finish date
	manga.finish_date = {};
	manga.finish_date.month =
		parseInt(/add_manga_finish_date_month.+\s.+value="(\d+)?"\s*selected="selected"/.exec(text)[1]) || '';
	manga.finish_date.day =
		parseInt(/add_manga_finish_date_day.+\s.+value="(\d+)?"\s*selected="selected"/.exec(text)[1]) || '';
	manga.finish_date.year =
		parseInt(/add_manga_finish_date_year.+\s.+value="(\d+)?"\s*selected="selected"/.exec(text)[1]) || '';
	// Status
...
function buildMyAnimeListBody(usePepper, manga, csrf, status = 1) {
	...
	if (usePepper) {
		...
		manga.status =
			manga.status == 2 || (manga.total_chapter > 0 && manga.currentChapter.chapter >= manga.total_chapter)
				? 2
				: status;
